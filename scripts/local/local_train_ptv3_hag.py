...
# o = rho*g^2; density-invariant for o>=1, breaks for o<1. D1 jitters the voxel grid in
# training; D5 averages density(scale) views at inference. D2b AdaBN omitted (PTv3 BN is
# only in pooling/stem, so BN-recalibration barely helps).
...
# 7->8 (retrain). Pair with DG_DENSITY_AUG so rho varies.
DG_LOGDK_FEAT  = False
DG_LOGDK_K     = 8
...
def train_ptv3(dataset: Optional[str] = None, grid: Optional[float] = None,
    ...
    def build_model(num_classes):
        ...
        backbone = PointTransformerV3(
            in_channels=7 + (1 if DG_LOGDK_FEAT else 0),   # xyz + rgb + HAG (+ log d_k, D3b)
    ...
    def to_ptv3_batch(tiles_for_batch, training=True):
        ...
        for tile in tiles_for_batch:
            ...
            # D1 density jitter: coarsen the voxel grid per tile (train only). hag is
            # sliced by `uniq` below, so it stays index-consistent. grid_coord still
            # comes from the dedup keys -> spconv submanifold invariant preserved.
                ...
                for tile in tiles:
                    ...
                    feat = np.concatenate(
                        [vx, rgb[first].astype(np.float32) / 255.0,
                         hag[first, None].astype(np.float32)]
                        + ([dg.local_density_logdk(vx, DG_LOGDK_K)[:, None]] if DG_LOGDK_FEAT else []),
                        axis=1).astype(np.float32)
                    coord = torch.from_numpy(vx).cuda()
                    featt = torch.from_numpy(feat).cuda()
