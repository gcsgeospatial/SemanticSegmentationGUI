"""
Local training script for RandLA-Net (PyTorch) — COLD-START + HAG variant.

The HAG twin of local_train_randlanet.py: identical except it appends a
HeightAboveGround feature as a 6th input channel (IN_DIM 5 -> 6 =
[xyz, intensity, return_number, HAG]); fc0 is rebuilt at 6->8. HAG is read
per point from the canonical scene .npz "hag" array when present; a dataset
without HAG falls back to a z-scene-min proxy so IN_DIM stays fixed at 6.
Run it head-to-head against local_train_randlanet.py to measure HAG's
contribution.
...
def train_randlanet(dataset: Optional[str] = None, sub_grid: Optional[float] = None,
    ...
    if dataset:
        ...
        ds_meta, NUM_CLASSES, CLASS_NAMES, HAG_SOURCE, PREP_DIR = {}, 0, [], "", None
    ...
    def load_canonical_hag(npz_path, xyz):
        # Real per-point HAG from the dataset's npz when present; else a
        # z-scene-min proxy so the 6th channel stays defined (IN_DIM=6).
        z = np.load(npz_path)
    ...
    def ensure_prep():
        ...
        for split, items in (("train", train_list), ("val", val_list),
                             ("test", test_list)):
            # HAG is read per point from the scene npz ("hag" array), with a
            # z-scene-min proxy fallback when the dataset has none.
    ...
    def make_predict_scene(net, num_classes):
        def _predict_scene(pc_path):
            ...
            # Infer-input npz tiles without a "hag" array -> z-scene-min proxy
            # for the 6th channel (best-effort demo).
