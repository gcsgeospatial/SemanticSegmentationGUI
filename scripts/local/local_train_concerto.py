...
default layout); weights are CC-BY-NC 4.0 and checkpoints embed the model config
...
def train_pcssl(dataset: Optional[str] = None, grid: Optional[float] = None,
    ...
    def build_feat(cxyz, rgbf, extras=None, drop=()):
        # the pretrained stem expects a trailing 3-wide (zeroed) normal slot
        return tc.ptv3_build_feat(FEAT_SPEC, cxyz, rgbf, extras, drop,
                                  DG_LOGDK_FEAT, DG_LOGDK_K, normal_block=True)

    def _stem_is_pretrained():
        # only the exact pretraining layout maps onto the pretrained stem
        return (FEAT_SPEC == FEAT_DEFAULT and not DG_LOGDK_FEAT)
