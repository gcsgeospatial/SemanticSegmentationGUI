"""
Modal shell for RandLA-Net (PyTorch), COLD-START + HAG — thin subprocess wrapper.

HAG twin of modal_train_randlanet.py: the local trainer appends an extra HAG
channel (IN_DIM 5 -> 6; fc0 rebuilt at 6->8). Canonical --dataset runs without a
real HAG feature fall back to a z-scene-min proxy for the 6th channel (handled in
the local trainer). Random initialization (no pretrained weights).

Provisions a GPU container + the outputs / terminal-datasets volumes, then shells
out to the local trainer (scripts/local/local_train_randlanet_hag.py) so local
