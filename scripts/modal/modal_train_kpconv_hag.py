"""
Modal shell for the ORIGINAL KPConv (deformable KPFCNN), COLD-START + HAG —
thin subprocess wrapper.

HAG twin of modal_train_kpconv.py: the local trainer feeds a real
HeightAboveGround channel instead of the tile-relative height proxy (same 4
input channels). Random init (no warm-start).

Provisions a GPU container + the outputs / terminal-datasets volumes, then
shells out to the local trainer (scripts/local/local_train_kpconv_hag.py) so
local and cloud run byte-identical code. Trains on a canonical trainer_gui
dataset passed via --dataset (staged on the terminal-datasets volume).
...
image = image.add_local_file("scripts/local/local_train_kpconv_hag.py", "/root/local_train_kpconv_hag.py")
# the hag entry point is a thin wrapper — ship the real trainer too
...
def train_kpconv(dataset: Optional[str] = None, mode: str = "train",
    ...
    trainer. All training/inference logic lives in local_train_kpconv_hag.py —
    this only shells out to it, so local and cloud run byte-identical code."""
    import subprocess
    import sys
    import threading

    cmd = [sys.executable, "/root/local_train_kpconv_hag.py"]
    for _flag, _val in (
        ("--dataset", dataset),
        ("--mode", mode),
        ("--weights", weights),
